...
describe("Todo CRUD operations", () => {
	...
	// TODO:
	test("Todos were listed and filtered by status.", async () => {
		// Create a new todo.
		const todo1 = await createTodo();
	...
	// TODO:
	test("Todos were listed and sorted by creation date.", async () => {
		// Create a new todo.
		const todo1 = await createTodo({
			createdAt: new Date(new Date().setDate(new Date().getDate() - 14)), // Two weeks ago from today
		});
		const todo2 = await createTodo({
			createdAt: new Date(new Date().setDate(new Date().getDate() + 7)), // A week from today
		});
		const todo3 = await createTodo({
			createdAt: new Date(new Date().setDate(new Date().getDate() - 7)), // A week ago from today
		});
